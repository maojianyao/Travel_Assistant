  ...
  <div class="result-container">
    ...
    <div class="page-header">
      ...
      <a-space size="middle">
        ...
        <!-- 复制分享链接 -->
        <a-button v-if="tripId" @click="copyShareLink" type="default">
          🔗 分享
        </a-button>
    ...
    <a-alert
      ...
      type="warning"
      show-icon
      message="AI 行程生成暂时失败, 当前展示的是示例行程, 建议返回首页重新生成"
      style="max-width: 1400px; margin: 0 auto 16px"
    />
        ...
        <a-card title="📅 每日行程" :bordered="false" class="days-card">
          <a-collapse v-model:activeKey="activeDays">
            ...
            >
                    ...
                    <a-card :title="item.name" size="small" class="attraction-card">
                      ...
                      <template #extra v-if="editMode">
                        <a-space>
                          ...
                          <a-button
                            size="small"
                            @click="openPoiModal(day.day_index, index, 'replace')"
                          >
                            🔄
                          </a-button>
                        </a-space>
              ...
              <!-- 编辑模式: 添加景点 -->
              <div v-if="editMode" class="add-attraction-row">
                <a-button
                  type="dashed"
                  block
                  @click="openPoiModal(day.day_index, day.attractions.length, 'add')"
                >
                  ➕ 添加景点
                </a-button>
              </div>
    ...
    <a-modal
      ...
      :title="poiModalMode === 'add' ? '➕ 添加景点' : '🔄 替换景点'"
      :ok-button-props="{ disabled: !selectedPoi }"
      ok-text="确认"
      @ok="confirmPoiSelect"
    >
      <a-input-search
        v-model:value="poiKeyword"
        placeholder="输入景点关键词, 如: 博物馆"
        enter-button="搜索"
        :loading="poiSearching"
        @search="handlePoiSearch"
        style="margin-bottom: 16px"
      />
      <a-empty v-if="!poiResults.length" description="暂无搜索结果" />
      <a-list v-else :data-source="poiResults" size="small" class="poi-list">
        <template #renderItem="{ item }">
          <a-list-item
            :class="['poi-item', { 'poi-item-active': selectedPoi && selectedPoi.id === item.id }]"
            @click="selectedPoi = item"
          >
...
import { getTripDetail, searchPoi } from '@/services/api'
import type { TripPlan, POIInfo, Attraction } from '@/types'
...
const poiModalMode = ref<'add' | 'replace'>('add')
const poiTargetDayIndex = ref(0)
const poiTargetAttrIndex = ref(0)
const poiKeyword = ref('')
const poiSearching = ref(false)
const poiResults = ref<POIInfo[]>([])
const selectedPoi = ref<POIInfo | null>(null)

onMounted(async () => {
  const routeTripId = route.query.id as string | null
  const cached = sessionStorage.getItem('tripPlan')

  if (routeTripId) {
    // 通过分享链接或历史记录进入: 优先从后端加载行程
    tripId.value = routeTripId
    try {
      const detail = await getTripDetail(routeTripId)
      if (detail.success && detail.data) {
        tripPlan.value = detail.data
        sessionStorage.setItem('tripPlan', JSON.stringify(detail.data))
      }
    } catch (e: any) {
      message.error(e.message || '行程加载失败')
    }
  } else if (cached) {
    tripId.value = sessionStorage.getItem('tripId')
  ...
  if (tripPlan.value) {
    ...
    // 地图初始化与景点图片加载并行执行, 图片就绪后响应式自动刷新, 不阻塞地图渲染
    initMap()
...
const recalcBudget = () => {
  ...
  tripPlan.value.days.forEach(day => {
    day.attractions.forEach(a => {
      totalAttractions += a.ticket_price || 0
    })
    day.meals.forEach(m => {
      totalMeals += m.estimated_cost || 0
    })
    if (day.hotel) {
      totalHotels += day.hotel.estimated_cost || 0
    }
  })

  budget.total_attractions = totalAttractions
  budget.total_meals = totalMeals
  budget.total_hotels = totalHotels
  budget.total = totalAttractions + totalMeals + totalHotels + (budget.total_transportation || 0)
}

// 复制分享链接
const copyShareLink = async () => {
  if (!tripId.value) {
    message.warning('当前行程暂不支持分享')
    return
  }
  const link = `${window.location.origin}/result?id=${tripId.value}`
  try {
...
// 打开添加/替换景点弹窗
const openPoiModal = (dayIndex: number, attrIndex: number, mode: 'add' | 'replace') => {
  poiModalMode.value = mode
  poiTargetDayIndex.value = dayIndex
  poiTargetAttrIndex.value = attrIndex
  poiKeyword.value = ''
  poiResults.value = []
...
const handlePoiSearch = async () => {
  if (!poiKeyword.value.trim() || !tripPlan.value) return
  poiSearching.value = true
  selectedPoi.value = null
  try {
    const res = await searchPoi(poiKeyword.value.trim(), tripPlan.value.city)
    poiResults.value = res.data || []
    if (!poiResults.value.length) {
      message.info('未搜索到相关POI, 请更换关键词')
    }
  } catch (e: any) {
    message.error(e.message || 'POI搜索失败')
  } finally {
...
// 确认选择POI并写入行程
const confirmPoiSelect = () => {
  if (!tripPlan.value || !selectedPoi.value) return
  const day = tripPlan.value.days[poiTargetDayIndex.value]
  ...
  if (poiModalMode.value === 'replace') {
    day.attractions.splice(poiTargetAttrIndex.value, 1, attraction)
    message.success(`已替换为「${poi.name}」`)
  } else {
    day.attractions.push(attraction)
    message.success(`已添加「${poi.name}」`)
  }
...
// 规范化折叠面板激活项(accordion模式遗留值可能是单值或null)
const normalizeActiveDays = (): number[] => {
  const v = activeDays.value as unknown
...
const exportAsImage = async () => {
  let prevActiveDays = normalizeActiveDays()
  try {
    message.loading({ content: '正在生成图片...', key: 'export', duration: 0 })

    // 折叠面板未展开的内容不在DOM中, 导出前临时展开全部天
    ...
    activeDays.value = prevActiveDays
  }
}
