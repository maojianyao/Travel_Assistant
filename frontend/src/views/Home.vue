<template>
  <div class="home-container">
    ...
    <!-- 最近生成的行程 -->
    <a-card v-if="recentTrips.length" class="form-card recent-card" :bordered="false" title="🕘 最近生成的行程">
      <a-list :data-source="recentTrips">
        <template #renderItem="{ item }">
          <a-list-item class="recent-item" @click="openTrip(item)">
            <a-list-item-meta
              :title="`${item.city} · ${item.travel_days}天行程`"
              :description="`${item.start_date} 至 ${item.end_date} · 生成于 ${item.created_at}`"
            />
            <template #actions>
              <a-tag v-if="item.is_fallback" color="orange">示例行程</a-tag>
              <span class="recent-arrow">查看 →</span>
            </template>
          </a-list-item>
        </template>
      </a-list>
...
import { generateTripPlanStream, listTrips } from '@/services/api'
...
// 最近生成的行程列表
const recentTrips = ref<TripListItem[]>([])

const loadRecentTrips = async () => {
  try {
    const res = await listTrips(6)
...
const openTrip = (trip: TripListItem) => {
  router.push(`/result?id=${trip.trip_id}`)
}

onMounted(() => {
  loadRecentTrips()
  // 用户填写表单期间预加载高德JS API(loader内部有缓存), 进入结果页可立即渲染地图
  AMapLoader.load({
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // 使用SSE流式接口, 进度条反映真实生成进度
    const response = await generateTripPlanStream(requestData, (percent, msg) => {
    ...
    loadingStatus.value = '✅ 完成!'
     if (response.success && response.data) {
      // 保存到sessionStorage(刷新兜底), trip_id用于分享与二次查看
      sessionStorage.setItem('tripPlan', JSON.stringify(response.data))
      if (response.trip_id) {
        sessionStorage.setItem('tripId', response.trip_id)
      }

      if (response.message === '旅行计划生成成功') {
      ...
      // 短暂延迟后跳转
      setTimeout(() => {
        router.push(response.trip_id ? `/result?id=${response.trip_id}` : '/result')
      }, 500)
    } else {
